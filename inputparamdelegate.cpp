...
InputParamDelegate::InputParamDelegate(QObject *parent)
    : QStyledItemDelegate(parent)
{}

void InputParamDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    painter->save();
    ...
    if (index.column() == InputParamModel::NameColumn) {
    ...
    } else if (index.column() == InputParamModel::VisibleColumn) {
    ...
    if (index.column() == InputParamModel::NameColumn && DepthForIndex(index) > 0 && !IsLastChildInParent(index)) {
...
QSize InputParamDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
...
bool InputParamDelegate::editorEvent(QEvent *event, QAbstractItemModel *model,
                                const QStyleOptionViewItem &option, const QModelIndex &index)
{
    ...
    if (event->type() == QEvent::MouseButtonRelease && index.column() == InputParamModel::VisibleColumn) {
    ...
    if (event->type() == QEvent::MouseButtonPress && index.column() == InputParamModel::NameColumn && HasChildren(index)) {
...
bool InputParamDelegate::IsSectionHeader(const QModelIndex &index) const
{
    return index.data(Qt::UserRole).toBool();
}

QWidget *InputParamDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option,
                                     const QModelIndex &index) const
{
    Q_UNUSED(option);

    if (index.column() != InputParamModel::StartColumn) {
        return nullptr;
    }
...
void InputParamDelegate::setEditorData(QWidget *editor, const QModelIndex &index) const
{
...
void InputParamDelegate::setModelData(QWidget *editor, QAbstractItemModel *model,
                                 const QModelIndex &index) const
{
    QString value;
...
void InputParamDelegate::updateEditorGeometry(QWidget *editor, const QStyleOptionViewItem &option,
                                         const QModelIndex &index) const
{
    Q_UNUSED(index);
...
int InputParamDelegate::DepthForIndex(const QModelIndex &index) const
{
...
QRect InputParamDelegate::ArrowRect(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    const int depth = DepthForIndex(index);
    const int indentWidth = depth > 0 ? depth * LevelIndent + 6 : 0;
    const int contentLeft = option.rect.left()
...
bool InputParamDelegate::HasChildren(const QModelIndex &index) const
{
    if (!index.isValid()) {
        return false;
    }
...
bool InputParamDelegate::IsLastChildInParent(const QModelIndex &index) const
{
    // 用于判断缩进区底部横线是否需要补齐。
