...
InputParamModel::InputParamModel(QObject *parent)
    : QAbstractItemModel(parent)
{
    SetupModelData();
}

InputParamModel::~InputParamModel()
{
    Clear();
}

void InputParamModel::Clear()
{
    beginResetModel();
...
QModelIndex InputParamModel::index(int row, int column, const QModelIndex &parentIndex) const
{
...
QModelIndex InputParamModel::parent(const QModelIndex &childIndex) const
{
...
int InputParamModel::rowCount(const QModelIndex &parentIndex) const
{
...
int InputParamModel::columnCount(const QModelIndex &parentIndex) const
{
    Q_UNUSED(parentIndex);
    return ColumnCount;
}

QVariant InputParamModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid()) {
        return QVariant();
    }
...
QVariant InputParamModel::headerData(int section, Qt::Orientation orientation, int role) const
{
...
bool InputParamModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    if (!index.isValid()) {
        return false;
    }
...
Qt::ItemFlags InputParamModel::flags(const QModelIndex &index) const
{
    if (!index.isValid()) {
...
void InputParamModel::ToggleExpand(const QModelIndex &index)
{
    if (!index.isValid()) {
        return;
    }
...
void InputParamModel::SortByColumn(int column, ParamSortMode mode)
{
...
void InputParamModel::SortItems(QVector<ParamItem*> &items, int column, ParamSortMode mode)
{
...
int InputParamModel::CompareItems(const ParamItem *left, const ParamItem *right, int column, ParamSortMode mode) const
{
...
int InputParamModel::CompareValues(const ParamItem *left, const ParamItem *right) const
{
...
int InputParamModel::ValueTypeRank(FmuValueType type)
{
    switch (type) {
    case FmuValueType::Boolean:
        return 0;
...
bool InputParamModel::BoolValue(const QString &value)
{
...
double InputParamModel::NumericValue(const QString &value, bool *ok)
{
    return value.trimmed().toDouble(ok);
}

QJsonObject InputParamModel::ToJson() const
{
...
void InputParamModel::FromJson(const QJsonObject &json)
{
    beginResetModel();
    qDeleteAll(m_rootItems);
    m_rootItems.clear();
...
QString InputParamModel::ToJsonString() const
{
    const QJsonDocument doc(ToJson());
    return QString::fromUtf8(doc.toJson(QJsonDocument::Indented));
}

bool InputParamModel::FromJsonString(const QString &jsonString)
{
...
void InputParamModel::SetupModelData()
{
    beginResetModel();
    qDeleteAll(m_rootItems);
    m_rootItems.clear();
...
ParamItem *InputParamModel::GetItem(const QModelIndex &index) const
{
    if (!index.isValid()) {
        return nullptr;
    }
...
void InputParamModel::AssignItemMetadata()
{
...
void InputParamModel::AssignItemMetadata(ParamItem *parent)
{
    if (!parent) {
        return;
    }
