...
void InputParamEditorWidget::SetupUI()
{
    ...
    m_delegate = new ParamDelegate(this);
    m_headerView = new CustomHeaderView(Qt::Horizontal, m_treeView);
    // 交付要求里当前只保留名称列排序，开始列不开放排序入口。
    m_headerView->SetSortableSections(QSet<int>{ParamModel::NameColumn});
...
void InputParamEditorWidget::ResizeColumnsToAvailableWidth()
{
    ...
    if (availableWidth <= MinTotalWidth) {
        m_treeView->setColumnWidth(ParamModel::NameColumn, MinNameWidth);
        m_treeView->setColumnWidth(ParamModel::VisibleColumn, MinVisibleWidth);
        m_treeView->setColumnWidth(ParamModel::StartColumn, MinStartWidth);
        m_treeView->setColumnWidth(ParamModel::UnitColumn, MinUnitWidth);
        return;
    }
    ...
    m_treeView->setColumnWidth(ParamModel::NameColumn, nameWidth);
    m_treeView->setColumnWidth(ParamModel::VisibleColumn, visibleWidth);
    m_treeView->setColumnWidth(ParamModel::StartColumn, startWidth);
    m_treeView->setColumnWidth(ParamModel::UnitColumn, unitWidth);
}

void InputParamEditorWidget::SetupConnections()
{
    connect(m_delegate, &ParamDelegate::ExpandClicked,
            this, &InputParamEditorWidget::OnExpandClicked);
    connect(m_delegate, &ParamDelegate::RowDoubleClicked,
...
void InputParamEditorWidget::LoadFromFile(const QString &filePath)
{
    ...
    if (!SetModelDataFromString(jsonString)) {
        QMessageBox::warning(this, QStringLiteral("加载失败"),
                             QStringLiteral("文件格式错误: %1").arg(filePath));
    }
}
...
void InputParamEditorWidget::RestoreExpandedRows(const QModelIndex &parentIndex)
{
    ...
    for (int row = 0; row < rows; ++row) {
        const QModelIndex index = m_model->index(row, ParamModel::NameColumn, parentIndex);
